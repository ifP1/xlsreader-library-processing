...
public class XlsxReader
{
	...
	*/
	...
	public XlsxReader ( PApplet thePapplet, String theXlsFile )
	{
		papplet = thePapplet;
		xlsFile = theXlsFile;
		...
		if ( is == null )
		{
			papplet.println( "ERR, XlsxReader(): file not found or unable to read" );
			return;
		}
		...
		{
			e.printStackTrace();
			papplet.println( "ERR, XlsxReader(): general error (" + e.getMessage() + ")" );
		}
	...
	*/
	...
	public int getInt ( int rowNum, int cellNum )
	{
		...
		if ( !isCellType( cell, XSSFCell.CELL_TYPE_NUMERIC )) papplet.println( "ERR, getInt(): wrong celltype" );
	...
	*/
	...
	{
		XSSFCell cell = getCell(rowNum,cellNum);
		
		if ( cell == null ) papplet.println( "ERR, getFloat(): cell is null" );
		
		if ( !isCellType( cell, XSSFCell.CELL_TYPE_NUMERIC )) papplet.println( "ERR, getFloat(): wrong celltype" );
	...
	*/
	...
	public String getString ( int rowNum, int cellNum )
	{
		...
		if ( !isCellType( cell, XSSFCell.CELL_TYPE_STRING )) papplet.println( "ERR, getString(): wrong celltype" );
	...
	*	@param	rowNum	Row (vertically) to read from. First row is 0.
	*	@param	cellNum Cell (horizontal) in the row to read from. Starts at 0.
	*	@return	XSSFCell The XSSFCell object.
	*	@see org.apache.poi.hssf.usermodel.XSSFCell
	*/
	
	private XSSFCell getCell (int _rowNum, int cellNum)
	{
	...
	private boolean isCellType (XSSFCell _cell, int type)
	{
		if ( _cell == null ) {
		...
		switch ( _cell.getCellType() )
		{
			case XSSFCell.CELL_TYPE_STRING: 
				papplet.println( "XSSFCell.CELL_TYPE_STRING" );
				break;
			case XSSFCell.CELL_TYPE_NUMERIC: 
				papplet.println( "XSSFCell.CELL_TYPE_NUMERIC" );
				break;
			case XSSFCell.CELL_TYPE_BLANK: 
				papplet.println( "XSSFCell.CELL_TYPE_BLANK" );
				break;
		}
		return false;
	}
